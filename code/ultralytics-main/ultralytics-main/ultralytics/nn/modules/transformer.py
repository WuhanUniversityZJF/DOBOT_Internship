...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.ma = nn.MultiheadAttention(c1, num_heads, dropout=dropout, batch_first=True)#ma:多头自注意力
    ...
    @staticmethod
    def with_pos_embed(tensor: torch.Tensor, pos: Optional[torch.Tensor] = None) -> torch.Tensor:#安全加法器，静态函数无需实例化
        """Add position embeddings to the tensor if provided."""
        return tensor if pos is None else tensor + pos

    def forward_post(#后归一化路径
        self,
        src: torch.Tensor,
...
class AIFI(TransformerEncoderLayer):#2D图像Transformer Encoder层
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        c, h, w = x.shape[1:]
        pos_embed = self.build_2d_sincos_position_embedding(w, h, c)#采用二维正弦-余弦位置编码
...
class TransformerLayer(nn.Module):#简化版的Transformer层，ViT风格
    ...
    def __init__(self, c: int, num_heads: int):
        ...
        self.ma = nn.MultiheadAttention(embed_dim=c, num_heads=num_heads)#ma：多头注意力
        self.fc1 = nn.Linear(c, c, bias=False)#fc1:第一层前馈网络
        self.fc2 = nn.Linear(c, c, bias=False)#fc2:第二层前馈网络
