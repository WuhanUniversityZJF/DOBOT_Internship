...
class WandbLogger():
    ...
    def setup_training(self, opt):
        ...
        if isinstance(opt.resume, str):
            ...
            if modeldir:
                ...
                opt.weights, opt.save_period, opt.batch_site, opt.bbox_interval, opt.epochs, opt.hyp, opt.imgsz = str(
                    self.weights), config.save_period, config.batch_site, config.bbox_interval, config.epochs,\
    ...
    def log_dataset_artifact(self, data_file, single_cls, project, overwrite_config=False):
        ...
        # log train set
        if not log_val_only:
            self.train_artifact = self.create_dataset_table(LoadImagesAndLabels(data['train'], rect=True, batch_site=1),
                                                            names,
        ...
        self.val_artifact = self.create_dataset_table(
            LoadImagesAndLabels(data['val'], rect=True, batch_site=1), names, name='val') if data.get('val') else None
        if data.get('val'):
