            ...
            with open(os.devnull, 'w') as devnull:
                with contextlib.redirect_stdout(devnull):
                    ...
                    if results:
                        ...
                        #修改后
                        if not hasattr(self.coco_gt, 'dataset'):
                            self.coco_gt.dataset = {}
                        self.coco_gt.dataset.setdefault('info', {})  # 添加缺失字段
                        coco_dt = self.coco_gt.loadRes(results)


                    else:
                        coco_dt = COCO()
                        coco_dt.dataset = {'images': [], 'annotations': [], 'categories': []}  # 添加必要字段
                        if hasattr(self.coco_gt, 'dataset'):
                            coco_dt.dataset.update(
                                {k: v for k, v in self.coco_gt.dataset.items() if k != 'annotations'})
